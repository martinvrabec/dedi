...
public class PhysicalSpacePlot extends AbstractBeamlineConfigurationPlotter {
	public PhysicalSpacePlot(IPlottingSystem<Composite> system, BeamlineConfigurationPlotView view) {
		super(system, view);
	}
	...
	protected void createDetectorRegion(){	
		...
		detectorROI = new RectangularROI(0, 0, 
				view.getDetector().getNumberOfPixelsX()*view.getDetector().getXPixelMM(), 
				view.getDetector().getNumberOfPixelsY()*view.getDetector().getYPixelMM(), 0);
		...
		if(system.isRescale()){
			...
			double ysize = view.getDetector().getNumberOfPixelsY()*view.getDetector().getYPixelMM();
			double xsize = view.getDetector().getNumberOfPixelsX()*view.getDetector().getXPixelMM();
	...
	protected void createBeamstopRegion(){
		...
		clearanceROI = new EllipticalROI(view.getClearance()*view.getDetector().getXPixelMM() + view.getBeamstop().getDiameterMM()/2, 
				view.getClearance()*view.getDetector().getYPixelMM() + view.getBeamstop().getDiameterMM()/2, 0, 
				view.getBeamstop().getXCentre()*view.getDetector().getXPixelMM(), 
				view.getBeamstop().getYCentre()*view.getDetector().getYPixelMM());
		beamstopROI = new CircularROI(view.getBeamstop().getDiameterMM()/2, view.getBeamstop().getXCentre()*view.getDetector().getXPixelMM(), 
				view.getBeamstop().getYCentre()*view.getDetector().getYPixelMM());
	...
	protected void createRay(){
		...
		if(!results.getIsSatisfied()){	
			...
			inaccessibleRangeROI = new LinearROI(new double[] {view.getBeamstop().getXCentre()*view.getDetector().getXPixelMM(), 
					view.getBeamstop().getYCentre()*view.getDetector().getYPixelMM()},
					                     new double[] {startPoint.x, 
	                								   startPoint.y});
			...
			visibleRangeROI1 = new LinearROI(new double[] {startPoint.x, 
										                   startPoint.y}, 
										      new double[] {reqestedRangeStartPoint.x, 
										    		        reqestedRangeStartPoint.y});
			
			visibleRangeROI2 = new LinearROI(new double[] {reqestedRangeEndPoint.x, 
														   reqestedRangeEndPoint.y}, 
										      new double[] {endPoint.x, 
										    		        endPoint.y});
			requestedRangeROI = new LinearROI(new double[] {reqestedRangeStartPoint.x,	
															reqestedRangeStartPoint.y}, 
					                          new double[] {reqestedRangeEndPoint.x,
					                        		        reqestedRangeEndPoint.y});
	...
	protected void createCameraTubeRegion(){
		if(cameraTubeRegion != null) system.removeRegion(cameraTubeRegion);
		...
		cameraTubeROI = new CircularROI(view.getCameraTube().getDiameterMM()/2, view.getCameraTube().getXCentre()*view.getDetector().getXPixelMM(), 
				view.getCameraTube().getYCentre()*view.getDetector().getYPixelMM());
